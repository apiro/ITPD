...
/**
 * This is socket factory used to create either plain sockets or SSL sockets
 * based on the target's policies and the client policies.
 * 
 * @author Vivek Nagar
...
public class IIOPSSLSocketFactory implements ORBSocketFactory {
	...
	/*
	 * this is stored for the Server side of SSL Connections. Note: There will
	 * be only a port per iiop listener and a corresponding ctx for that port
	 */
	/*
	...
	/*
	 * this is stored for the client side of SSL Connections. Note: There will
	 * be only 1 ctx for the client side, as we will reuse the ctx for all SSL
	 * connections
	 */
	...
	public IIOPSSLSocketFactory() {
		try {
			...
			boolean notServerOrACC = Globals.getDefaultHabitat() == null ? true
					: false;
			if (!notServerOrACC) {
			...
			if ((processType != null) && (processType.isServer())) {
				...
				Config conf = Globals.getDefaultHabitat().getService(
						Config.class, ServerEnvironment.DEFAULT_INSTANCE_NAME);
				IiopService iiopBean = conf
						.getExtensionByType(IiopService.class);
				...
				for (IiopListener listener : iiopListeners) {
					...
					boolean securityEnabled = Boolean.valueOf(listener
							.getSecurityEnabled());

					if (securityEnabled) {
						if (ssl != null) {
							boolean ssl2Enabled = Boolean.valueOf(ssl
									.getSsl2Enabled());
							boolean tlsEnabled = Boolean.valueOf(ssl
									.getTlsEnabled());
							boolean ssl3Enabled = Boolean.valueOf(ssl
									.getSsl3Enabled());
							sslInfo = init(ssl.getCertNickname(), ssl2Enabled,
									ssl.getSsl2Ciphers(), ssl3Enabled,
									ssl.getSsl3TlsCiphers(), tlsEnabled);
						} else {
							sslInfo = getDefaultSslInfo();
						}
						portToSSLInfo.put(new Integer(listener.getPort()),
								sslInfo);
					}
				}
				...
				iiopBean.getSslClientConfig().getSsl() != null) {
					...
					if (outboundSsl != null) {
						boolean ssl2Enabled = Boolean.valueOf(outboundSsl
								.getSsl2Enabled());
						boolean ssl3Enabled = Boolean.valueOf(outboundSsl
								.getSsl3Enabled());
						boolean tlsEnabled = Boolean.valueOf(outboundSsl
								.getTlsEnabled());
				...
				if ((processType != null) && (processType == ProcessType.ACC)) {
					IIOPSSLUtil sslUtil = Globals.getDefaultHabitat()
							.getService(IIOPSSLUtil.class);
					AppClientSSL clientSsl = (AppClientSSL) sslUtil
							.getAppClientSSL();
					if (clientSsl != null) {
						clientSslInfo = init(clientSsl.getCertNickname(),
								clientSsl.getSsl2Enabled(),
								clientSsl.getSsl2Ciphers(),
								clientSsl.getSsl3Enabled(),
	...
	/**
	 * serveralias/clientalias cannot be set at the same time. this method
	 * encapsulates the common code for both the client side and server side to
	 * create a SSLContext it is called once for each serveralias and once for
	 * each clientalias
	 */
	private SSLInfo init(String alias, boolean ssl2Enabled, String ssl2Ciphers,
			boolean ssl3Enabled, String ssl3TlsCiphers, boolean tlsEnabled)
			throws Exception {
		...
		if (tlsEnabled || ssl3Enabled) {
			ssl3TlsCipherArr = getEnabledCipherSuites(ssl3TlsCiphers, false,
					ssl3Enabled, tlsEnabled);
		}

		String[] ssl2CipherArr = null;
		if (ssl2Enabled) {
			ssl2CipherArr = getEnabledCipherSuites(ssl2Ciphers, true, false,
					false);
		}

		SSLContext ctx = SSLContext.getInstance(protocol);
		if (Globals.getDefaultHabitat() != null) {
			IIOPSSLUtil sslUtil = Globals.getDefaultHabitat().getService(
					IIOPSSLUtil.class);
			KeyManager[] mgrs = sslUtil.getKeyManagers(alias);
			ctx.init(mgrs, sslUtil.getTrustManagers(),
	 ...
	 * type of the server socket (SSL, SSL_MUTUALAUTH, PERSISTENT_SSL or
	 * CLEAR_TEXT).
	 * 
	 * @param type
	...
	public ServerSocket createServerSocket(String type,
		...
		if (type.equals(SSL_MUTUALAUTH) || type.equals(SSL)
			...
			if (orb.getORBData().acceptorSocketType()
					.equals(ORBConstants.SOCKETCHANNEL)) {
	...
	/**
	 * Create a client socket for the specified InetSocketAddress. Creates an
	 * SSL socket if the type specified is SSL or SSL_MUTUALAUTH.
	 * 
	 * @param type
	 * @param inetSocketAddress
	 * @return the socket.
	...
	public Socket createSocket(String type, InetSocketAddress inetSocketAddress)
		...
		try {
			...
			if (_logger.isLoggable(Level.FINE)) {
				_logger.log(Level.FINE, "createSocket(" + type + ", " + host
						+ ", " + port + ")");
			}
			if (type.equals(SSL) || type.equals(SSL_MUTUALAUTH)) {
				...
				Socket socket = null;
				if (_logger.isLoggable(Level.FINE)) {
					_logger.log(Level.FINE, "Creating CLEAR_TEXT socket for:"
							+ port);
				}

				if (orb.getORBData().connectionSocketType()
						.equals(ORBConstants.SOCKETCHANNEL)) {
					SocketChannel socketChannel = ORBUtility
							.openSocketChannel(inetSocketAddress);
	...
	/**
	 * Create an SSL server socket at the specified InetSocketAddress. If the
	 * type is SSL_MUTUALAUTH then SSL client authentication is requested.
	 */
	private ServerSocket createSSLServerSocket(String type,
		...
		if (inetSocketAddress == null) {
			throw new IOException(getFormatMessage(
					"iiop.invalid_sslserverport", new Object[] { null }));
		}
		int port = inetSocketAddress.getPort();
		...
		if (sslInfo == null) {
			throw new IOException(getFormatMessage(
					"iiop.invalid_sslserverport", new Object[] { iport }));
		}
		SSLServerSocketFactory ssf = sslInfo.getContext()
	...
	private Socket createSSLSocket(String host, int port) throws IOException {
		...
		try {
			...
			if (_logger.isLoggable(Level.FINE)) {
				_logger.log(Level.FINE, "Creating SSL Socket for host:" + host
						+ " port:" + port);
			}
			...
			IOException e2 = new IOException(
					"Error opening SSL socket to host=" + host + " port="
							+ port);
			e2.initCause(e);
			throw e2;
		}
	 ...
	 * Input is the cipherSuiteStr from xml which a space separated list ciphers
	 * with a prefix '+' indicating enabled, '-' indicating disabled. If no
	 * cipher is enabled, then it returns an empty array. If no cipher is
	 * specified, then all are enabled and it returns null.
	 * 
	 * @param cipherSuiteStr
	...
	private String[] getEnabledCipherSuites(String cipherSuiteStr,
		...
		if (cipherSuiteStr != null && cipherSuiteStr.length() > 0) {
				...
				if (cipherAction.startsWith("+")) {
					...
					if (cipherInfo != null
						...
						throw new IllegalStateException(getFormatMessage(
								"iiop.unknown_cipher", new Object[] { cipher }));
					}
			...
			cipherArr = (String[]) cipherList.toArray(new String[cipherList
					.size()]);
		}
		return cipherArr;
	}
	 ...
	 * 
	 * @param enableCiphers
	 *            ciphers enabled by socket factory
	...
	private boolean isValidProtocolCipher(CipherInfo cipherInfo,
			boolean ssl2Enabled, boolean ssl3Enabled, boolean tlsEnabled) {
		return (tlsEnabled && cipherInfo.isTLS() || ssl3Enabled
				&& cipherInfo.isSSL3() || ssl2Enabled && cipherInfo.isSSL2());
	}
	 ...
	 * @param params
	 *            the parameter array of Object
	 * @return the format String for _logger
	 */
	private String getFormatMessage(String key, Object[] params) {
		return MessageFormat.format(_logger.getResourceBundle().getString(key),
				params);
	}
